...
import { getRequestToken, getAuthorizationUrl, getAccessTokenFromCode, fetchArticles, getRequestTokenAndAuthUrl } from "../app/api/PocketApi";
...
const Blog = () => {
  ...
  useEffect(() => {
    const handlePocketAuth = async () => {
      try {
        ...
        if (!localStorage.getItem('pocketAccessToken') && authCode && storedRequestToken) {
          setLoading(true);
          ...
          router.replace('/blog'); // Clean the URL
        } else if (!localStorage.getItem('pocketAccessToken') && storedRequestToken) {
            ...
            console.warn("[Blog] No auth code in URL, but request token found in localStorage. Restarting auth.");
  ...
  const startPocketAuth = async () => {
    try {
      ...
      setError("Failed to start authentication process with Pocket. Please try again.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.blogContainer}>
      ...
      {loading && <div className={styles.loading}>Loading articles from Pocket...</div>}
      ...
      {articles.map((article) => (
        <div key={article.item_id} className={styles.blogItem}>
          <h2>
            <a href={article.resolved_url} target="_blank" rel="noopener noreferrer" className={styles.blogLink}>
              {article.resolved_title || "Untitled Article"} <span className="arrow">→</span>
            </a>
          </h2>
          
          {article.top_image_url && (
            <img src={article.top_image_url} alt={article.resolved_title} className={styles.articleImage} />
          )}
          
          <p className={styles.articleExcerpt}>{article.excerpt}</p>
          <div className={styles.articleMeta}>
            <span className={styles.articleDate}>Saved on {formatDate(article.time_added)}</span>
          </div>
        </div>
      ))}
    </div>
